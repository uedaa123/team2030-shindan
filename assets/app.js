...
var Q = {
 /* 入口。名前とメールを必須で取る。
    ここで取っておかないと、途中でやめた人が誰なのか分からず、
    運営が声をかけられない（それが本来いちばん拾いたい人）。
    結果は最後に自動でメール送信するので、そのことを先に伝える。
    送り先がない（logEndpoint が空）ときは、この設問ごと出さない。 */
 who: {key:"who", type:"form",
   label:"あなた",
   q:"お名前とメールアドレス",
   h:"診断が終わったら、結果をこのアドレスにお送りします。",
   fields:[
     {key:"name",  label:"お名前",         type:"text",  ph:"山田 太郎",       auto:"name"},
     {key:"email", label:"メールアドレス", type:"email", ph:"you@example.com", auto:"email"}
   ]},
...
/* 送り先があるときだけ、入口の設問を先頭に足す */
if (CFG.logEndpoint) {
  STEP_SETS.full  = [Q.who].concat(STEP_SETS.full);
  STEP_SETS.short = [Q.who].concat(STEP_SETS.short);
}
...
/* ── 状態 ───────────────────────────────────────── */
var A, step, STEPS, TOTAL, DB = null, lastResult = null, started = false, mailSent = false;
var SESSION = "s" + Date.now().toString(36) + Math.random().toString(36).slice(2, 8);

function reset(){
  A = {group:[], genre:[], project:null, wd:null, yn1:null, name:"", email:""};
  step = 0; lastResult = null; mailSent = false;
}
reset();
...
function form(S){
  ...
  '<div class="step">' +
    ...
    S.fields.map(function(f){
      return '<div class="namefield"><label for="f-' + f.key + '">' + esc(f.label) + '</label>' +
        '<input type="' + f.type + '" id="f-' + f.key + '" data-k="' + f.key + '" ' +
        'autocomplete="' + f.auto + '" maxlength="120" ' +
        (f.type === "email" ? 'inputmode="email" ' : '') +
        'placeholder="' + esc(f.ph) + '" value="' + esc(A[f.key]) + '"></div>';
    }).join("") +
    '<p class="copied" id="formmsg" role="status"></p>' +
    (step > 0 ? '<div class="nav"><button type="button" class="back" id="back">ひとつ戻る</button></div>' : '') +
  '</div>' +
  '<div class="bottombar"><div class="bottominner">' +
    '<button type="button" class="go" id="go"' + (whoReady() ? "" : " disabled") + '>' +
    (whoReady() ? "はじめる" : "2つとも入れてね") + '</button>' +
  '</div></div>';

  Array.prototype.forEach.call(app.querySelectorAll("input[data-k]"), function(inp){
    inp.oninput = function(){
      A[inp.getAttribute("data-k")] = inp.value.trim();
      var g = document.getElementById("go");
      g.disabled = !whoReady();
      g.textContent = whoReady() ? "はじめる" : "2つとも入れてね";
  ...
  document.getElementById("go").onclick = function(){
    if (!whoReady()) {
      document.getElementById("formmsg").textContent =
        !A.name ? "お名前を入れてください。" : "メールアドレスを確かめてください。";
      return;
    }
    markStarted();     /* ここで運営側に「誰が始めたか」が残る */
    step++; render(); top();
  };
  var bk = document.getElementById("back");
  if (bk) bk.onclick = function(){ step--; render(); top(); };
...
function whoReady(){ return !!A.name && validEmail(A.email); }
...
function results(){
  ...
  '<div class="res">' +
    ...
    '<p class="hint">バディやチームに共有しましょう。そして、アドバイスや感想をもらいましょう。' +
...
/* ── メールの状態 ───────────────────────────────────
   入口でアドレスをもらっているので、結果が出た時点で自動で送る。
   ここは「送ったこと」を見せて、届かなかった人が送り直せる場所。 ── */
function mailCard(){
  if (!CFG.logEndpoint || !A.email) return "";
  return '<div class="act mail">' +
    '<h4>メールでも送りました</h4>' +
    '<p>' + esc(A.email) + ' に、この結果を送っています。あとから見返せます。</p>' +
    '<p class="copied" data-msg="mail" role="status">送信中…</p>' +
    '<button type="button" class="go ghost mailbtn">もう一度送る</button>' +
  '</div>';
}
...
/* 結果が出たら1回だけ自動で送る。押し直したいときは「もう一度送る」から */
function autoMail(){
  if (!CFG.logEndpoint || !A.email) return;
  if (mailSent) {
    var m = app.querySelector('.copied[data-msg="mail"]');
    if (m) m.textContent = "送信ずみです。";
    return;
  }
  mailSent = true;
  sendMail(app.querySelector(".mailbtn"));
}
...
function sendMail(btn){
  ...
  if (!msg) return;
  if (!validEmail(A.email)) { msg.textContent = "メールアドレスを確かめてください。"; return; }
  ...
  function done(ok, note){
    ...
    if (!ok) mailSent = false;   /* 失敗したら、もう一度押せるようにしておく */
  }
  ...
  fetch(CFG.logEndpoint, {
    ...
    .then(function(t){
      if (/^ok/i.test(t)) done(true, "送りました。数分で届きます。見つからないときは迷惑メールも見てください。");
      else if (/limit/i.test(t)) done(false, "今日の送信上限に達しました。上のコピーを使ってください。");
      else done(false, "送れませんでした。上のコピーを使ってください。");
    })
    .catch(function(){
      fetch(CFG.logEndpoint, {
